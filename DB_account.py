...
def CreateNewAccount(acc_id, acc_name): # create new account for useeeee
...
def GetElementByElement(element_to_get, element_get_by, value_get_by):
    data.execute("SELECT %s FROM Accounts WHERE %s = '%s'" % (element_to_get, element_get_by, value_get_by))
